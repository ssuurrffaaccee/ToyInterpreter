...
def ValueOf(Exp,Env):
    ...
    def MulExpHandler(MulExp,Env):
        ...
        Exp2=Children[1]
        return NumberToVal(ValToNumber(ValueOf(Exp1,Env))*ValToNumber(ValueOf(Exp2,Env)))
    def ProcExpHandler(ProcExp,Env):
        assert ProcExp["ExpType"]=="Proc"
        Children=ProcExp["Children"]
        VarExp=Children[0]
        Body=Children[1]
        return ProcToVal(VarExp,Body,Env)
    def CallExpHandler(CallExp,Env):
        assert CallExp["ExpType"]=="Call"
        Children=CallExp["Children"]
        #Call
        VarExp=Children[0]
        Exp=Children[1]
        #Rroc
        ProcVal=ApplyEnv(Env,VarExp["Load"])
        Proc=ValToProc(ProcVal)
        ParamterVarExp=Proc[0]
        Body=Proc[1]
        ProcEnv=Proc[2]
        NewEnv=ExtendEnv(ParamterVarExp["Load"],ValueOf(Exp,Env),ProcEnv)
        return ValueOf(Body,NewEnv)
    def LetRecExpHandler(LetRecExp,Env):
        assert LetRecExp["ExpType"]=="LetRec"
        Children=LetRecExp["Children"]
        #LetRec
        ProcVar=Children[0]
        Parameter=Children[1]
        ProcBody=Children[2]
        LetBody=Children[3]
        NewEnv=ExtendEnv(ProcVar["Load"],ProcToVal(Parameter,ProcBody,Env),Env)
        ApplyEnv(NewEnv,ProcVar["Load"])["Value"][2]=NewEnv
        return ValueOf(LetBody,NewEnv)
